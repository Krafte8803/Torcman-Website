function initFahrwerkHelper() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!daten.length) {
      result.textContent =
        "Daten noch nicht geladen, bitte kurz warten und erneut versuchen.";
      return;
    }
    const startgewicht = parseFloat(
      document.getElementById("startgewicht").value,
    );
    const propellerZoll = parseFloat(
      document.getElementById("propeller").value,
    );
